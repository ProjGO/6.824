...
# if ! go test -timeout 30s -c -o tester; then
...
for i in $(seq "$((success+failed+1))" "$runs"); do
	...
	if [[ -z "$test" ]]; then
		./tester -test.v 2> "./manyLog/test-${i}.err" > "./manyLog/test-${i}.log" &
		pid=$!
	else
		./tester -test.run "$test" -test.v 2> "./manyLog/test-${i}.err" > "./manyLog/test-${i}.log" &
		pid=$!
	fi
